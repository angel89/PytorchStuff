# -*- coding: utf-8 -*-
"""Copy of DUDL_metaparams_optimizersComparison.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1n5eBeUn1U619b09eL8qc7nRWLW6MF5WW
...
def createTheQwertyNet(optimizerAlgo,learnRate):
  ...
  optimizer = optifun(net.parameters(),lr=learnRate)
...
optim = createTheQwertyNet('RMSprop', float(0.1))[2]
optim

"""# A function that trains the model"""

def function2trainTheModel(optimizerType, learnRate):

  # number of epochs
  numepochs = 50

  # create a new model
  net,lossfun,optimizer = createTheQwertyNet(optimizerType,learnRate)
  ...
  for epochi in range(numepochs):
    ...
    #FFS, y here are the labels (not the y coordinates), the x has both y and x
    for X,y in train_loader:
      ...
      # compute accuracy
      matches = torch.argmax(yHat,axis=1) == y     # booleans (false/true), argmax returns the index not the vlue, thats why it works
...
optos = ['SGD','RMSprop','Adam']
# average performance
learnRates = np.logspace(np.log10(.0001),np.log10(.1),20)
performance = np.zeros((len(learnRates), len(optos)))

for opto_indx, opto in enumerate(optos):

  for learnRate_indx, learnRate in enumerate(learnRates):
    trainAcc,testAcc,losses,net = function2trainTheModel(opto, learnRate)
    #plotTheResults(opto)

    # store the final results
    train = np.mean(trainAcc[-10:])
    test  = np.mean(testAcc[-10:])
    performance[learnRate_indx, opto_indx] = test
    #performance.append( f'{opto}: train {train:.1f}%, test {test:.1f}%' )

plt.plot(learnRates, performance,'o-', linewidth=2)
# plot the loss function
plt.legend(optos)

plt.xscale('log')

plt.ylabel('Average train perf')
plt.xlabel('learningrates')
